...
export function spawn(command: string, options?: ExecOptions): E.EventHandler {
    ...
    if (!command) throw new Error(`spawn() - No command specified`)
    ...
    function injectArgv(e: E.LiveTerminalCommandEvent, string: string): string {
        ...
        for (let i = 1; i < indexes.length; i++) 
            if (indexes[i] - 1 > indexes[i - 1]) 
                throw new SyntaxError(`spawn() - Placeholder {${indexes[i]}} allows for unused command parameters.\n`)
        
        if (matches.length > e.terminal.argv.length) throw new Error(`spawn() - Not all command parameters were filled.`)
    ...
    return (event) => new Promise<null | Error>(async end => {
        try {
            ...
            const child = cp.spawn(command, { ...o, stdio: o.stdio === 'ignore' ? ['ignore', 'ignore', 'ignore'] : ['inherit', 'inherit', 'inherit'] })

            child.on('exit', (code) => {
                if (o.acceptCodes?.includes(code!)) end(null)
                else end(new Error(`spawn() - Ended with exit code ${code!}.\nCommand: ${command}`))
            })
        } 
        catch (error) {
            end(error as Error)
